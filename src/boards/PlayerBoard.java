...
public class PlayerBoard extends Board{
	...
	public PlayerBoard()
	{
		...
		VBox vbox1 = new VBox(20, finalMessage, getCoordinadesLabel(), timeLabel, placeShipsMenu, getTextLabel(), getRoundLabel());
	...
	/**
	 * Get total game session time in seconds (or 0 if game is not ended)
	 * 
	 * @return totalTimeInSec : double that represents total game session time in seconds
	 */
	...
	/**
	 * Get a current game session time in seconds 
	 * 
	 * @return currentTime : double that represents a current game session time in seconds
	 */
	...
	private void startTimer()
	{
    	...
    	timeline.getKeyFrames().add(new KeyFrame(
    	        Duration.millis( 500 ),
    	        event -> {
    	            currentTime = System.currentTimeMillis() - startingTime; //get difference as a current time
    	            if (currentTime < 0 ) //in case of error set to 0
    	            {    	              
    	                timeLabel.setText(String.valueOf(0));
    	            } 
    	            else
    	            {
    	                timeLabel.setText("Timer: " + String.valueOf(currentTime/1000));
    	            }
    	        }
    	    ));
    	
    	timeline.setCycleCount( Animation.INDEFINITE );
    	timeline.play(); //start counting
	}
    ...
    public void endGame()
    {
    	//stop the timer, record the total time
    	endTime = System.currentTimeMillis();
    	totalTimeInSec = (endTime - startTime)/1000;
    	stopTimer();
